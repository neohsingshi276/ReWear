...
const Exchange = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // If the protected endpoints fail (e.g., 401/403), show a clear error and fall back to public list
      const msg = err?.response?.data?.error || err?.message || 'Failed to load exchange items';
      setError(msg);
    ...
    <div className="container mx-auto p-8">
      ...
      {error && (
        <div className="mb-6 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
          <b>Exchange load error:</b> {error}
          <div className="text-sm text-red-600 mt-1">
            Tip: make sure you are logged in and the backend returns items at <code>/api/exchange-products</code>.
          </div>
        </div>
      )}
            ...
            <div className="p-6">
              ...
              <div className="flex gap-4 mb-6 bg-gray-50 p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{selectedProduct.brand} | {selectedProduct.size} | RM {Number(selectedProduct.price).toFixed(2)}</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-64 overflow-y-auto">
                  {myProducts.map(product => (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500">RM {Number(product.price).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              )}
